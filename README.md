...
「5時間の動画を渡して『5分にハイライト編集して。テロップや装飾も文脈にあわせてつけて』と言うだけで自律的に編集する」——これを実現するパイプラインを搭載しています。

素材の **🤖 Auto-edit** ボタン（またはAPI）に指示文と目標尺を渡すと、以下を自動実行します:

1. **文字起こし（context取得）** — 添付字幕（SRT/VTT）→ ASR（音声認識）→ なし、の優先順で取得
2. **ハイライト選定＋テロップ/装飾の立案** — AIプランナーが、重要な発話区間を目標尺ぶん選び、文脈に合うテロップ（telop）と装飾を設計
3. **セグメント切り出し** — ffmpeg で採用区間だけを再エンコード（フレーム精度・連結可能）
4. **プロジェクト自動生成** — クリップを時系列に並べ、テロップ・装飾をタイムラインに配置
5. **レンダリング** — HyperFrames で MP4 化

生成物は通常のプロジェクトなので、**編集画面で人手で微調整**してから再レンダリングもできます。

### AIバックエンド（プラグイン式）

| 役割 | 使うもの | 有効化 | 未設定時 |
| --- | --- | --- | --- |
| ハイライト選定・テロップ執筆 | **Claude**（`claude-opus-4-8`） | `ANTHROPIC_API_KEY` | 決定論的ヒューリスティック（顕著性スコアリング）で代替 |
| 音声認識（字幕生成） | OpenAI互換 Whisper | `OPENAI_API_KEY`（＋任意 `OPENAI_BASE_URL`） | 字幕アップロード or 文字起こしなし（シーン/等間隔選定） |

`GET /api/autoedit/capabilities` で現在有効なバックエンドを確認できます。**キーが無くてもパイプライン全体は動作**します（テロップ文言や選定精度がAI有効時より単純になるだけ）。

### 5時間 → 5分 の実現性

- **選定の質**: 文脈理解が要るため、実運用では Claude プランナー（`ANTHROPIC_API_KEY`）を推奨。5時間ぶんの文字起こしは長いので、`transcript.js` はASR音声を10分単位にチャンク分割し、長大な文字起こしは自動で先頭からトリムしてプロンプトに収めます。
- **速度**: 5時間を丸ごとエンコードせず、**採用した約5分ぶんだけを切り出し再エンコード**するため現実的な時間で完了します（文字起こしが最長工程）。
- **字幕（テロップ）**: 文脈一致のテロップには文字起こしが必須。ASRキーが無い場合は、書き起こし/字幕ファイルの添付で同等の結果になります。
- **装飾**: HyperFrames の HTML/CSS/GSAP でタイトルカード・ローワーサード・アクセントバー・コーナーラベル等をプリセット提供。
...
- **超長尺のプラン**: 文字起こしが Claude のコンテキストに収まらないほど長い場合、`planner.js` を map-reduce 化（チャンクごとに候補抽出→最終選定）すると精度と安定性が上がります。セグメント切り出しは並列化可能。
